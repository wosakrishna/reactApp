...
const RepertSentence = ({ hours = 0, minutes = 0, seconds = 0 }) => {
  ...
  const [[h, m, s], setTime] = React.useState([hours=0, minutes=0 ,seconds=8]);
...
const reset = () => {
  setTime([parseInt(hours), parseInt(0), parseInt(8)]);
    setOver(false);
    stop()
...
const restart=(e)=>{
  e.preventDefault();
  setPaused(false)
  setTime([parseInt(hours), parseInt(minutes), parseInt(8)]);
  myRef.current.play();
...
//   const Adu=(e)=>{
//       e.preventDefault()
//     const audio = new Audio("../audio/re1.mp3")
//     audio.addEventListener('loadedmetadata', (e) => {
//       const data=Math.round(e.target.duration);
//     Sound.current=data;
    ...
    <div>
      ...
      <div className="container border border-dark  ">
        ...
        // <button id="auto" onClick={startAudio}>start</button>
          ...
          <Link to="/RepertSentence2" className=" btn btn-success float-end mx-5 my-5  ">Next</Link>
...
export default RepertSentence;
// import React,{useState,useEffect,useRef} from 'react'
// function RepertSentence() {
